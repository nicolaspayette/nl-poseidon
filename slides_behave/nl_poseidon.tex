...
\title{\LARGE ABM and environmental policy}
\subtitle{A mini-POSEIDON model in NetLogo}

\date{\scriptsize Behave Summer School\\Brescia, Italy\\\today}
\author{\Large Nicolas Payette}
\institute{%
  \includegraphics[height=20mm]{images/2024_oxrse_next_to_oxford.pdf}
}
...
\begin{frame}{What's the plan?}
  ...
  \begin{itemize}
    ...
    \item A little bit about the POSEIDON\footnote{\textbf{P}rocess-based \textbf{O}cean-system \textbf{S}imulator for \textbf{E}volving \textbf{I}ntegrated \textbf{D}omains and \textbf{O}perational \textbf{N}eeds. \quad\ldots\quad Yeah. I know.} fisheries ABM
    \item A lot about \hl{building} a simple NetLogo version of it.
    \item A fair bit about \hl{calibrating} the model\\and using it for \hl{optimizing} a policy.
  \end{itemize}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.575\textwidth}
      ...
      \begin{itemize}
        ...
        \item Integration with European Digital Twin Ocean
      \end{itemize}
      \vskip 1em
...
\begin{frame}[fragile=singleslide]{Estimating EEI parameters}
  \begin{columns}[T]
    \column{0.53\textwidth}\footnotesize
    \hl{Parameter Specification:}
    \vskip1mm
    \begin{minted}[fontsize=\scriptsize]{nlogo} 
...
\begin{frame}[fragile]{Goodness of fit}
  \begin{columns}
    ...
    \vskip15mm
    \begin{minted}[fontsize=\footnotesize]{nlogo}  
...
\begin{frame}[fragile=singleslide]{Generalizing BehaviorSearch load procedure}
    \begin{minted}[highlightlines={1,5,17-19,21-23},fontsize=\tiny]{nlogo}  
